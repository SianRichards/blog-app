        ...
        firebase.auth().signOut().then(() => {
            ...
            .catch((error) => {
                if (!this.state.errorMessages.includes(error.message)) {
                    this.setState(prevState => ({ errorMessages: [...prevState.errorMessages, error.message] }))
                }
            })
    };
    ...
    render() {
        return (
            <div onClick={this.signOutUser} className={this.props.className}>
                ...
                Sign Out
                {this.state.errorMessages && this.state.errorMessages.map((errorMessage) => {
                    return <div key={errorMessage} className="ui error message">{errorMessage}</div>
                })}
            </div>
        )
    }
