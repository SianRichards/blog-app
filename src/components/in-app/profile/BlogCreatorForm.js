...
class BlogCreatorForm extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.blogPostId !== this.props.blogPostId) {
      if (this.props.blogPostId !== "") {
        databaseRef("posts/" + this.props.blogPostId).on(
          "value",
          (snapshot) => {
            if (snapshot.val()) {
              this.setState({
                titleValue: snapshot.val().postTitle,
                contentValue: snapshot.val().postContent,
              });
            }
          }
        );
      ...
      if (this.props.updateBlogPost) {
        ...
        updateDatabaseValue("posts/" + this.props.blogPostId, {
          postTitle: this.state.titleValue,
          postContent: this.state.contentValue,
          updatedTimeStamp: currentDateAndTime,
        });
  ...
  saveBlogPost = () => {
    if (
      this.state.validFields.length === this.fields.length &&
      ...
      addDatabaseValue("posts", {
        ...
        let postStoreSuccessMessage = (
          <div
            className="ui success message message"
        ...
        this.setState((prevState) => ({
          messages: [...prevState.messages, postStoreSuccessMessage],
          titleValue: "",
          contentValue: "",
        }));
      });
  ...
  updateBlogPost = () => {
    if (
      ...
      let modalInfo = {
        ...
        modalMessage:
          "Updating this post will overwrite its previous content. Are you sure you want to update this post?",
      };
  ...
  renderInputFields = () => {
    this.fields = [
      {
        ...
        message: (
          <div className="ui error message">
            Post content cannot be left blank
          </div>
        ),
        isValid:
          this.state.contentValue !== "" &&
          this.state.contentValue !== "<p><br></p>"
            ? true
            : false,
    ...
    return this.fields.map((field) => {
      return (
        <div
          key={field.id}
          style={{ height: "375px", backgroundColor: "white", margin: "15px" }}
        >
          <ReactQuill
            theme="snow"
            style={{
