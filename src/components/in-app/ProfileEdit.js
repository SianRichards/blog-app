    ...
    const postProfileInformation = () => {
        ...
        orderDatabaseValuesByChild('profileInformation', 'userUID', authUID).once('value', (snapshot) => {
            if (snapshot.val()) {
                ...
                updateDatabaseValue('profileInformation/' + profileInformationID, { profileDescription: editValue })
            }
            // if they haven't already posted profile info
            // then add a new entry for that user and their
            // profile info
            else addDatabaseValue('profileInformation', { profileDescription: editValue, userUID: authUID })
        }).then(() => {
